...
layui.define('view', function(exports){
  ...
  //,ICON_SHRINK = 'layui-icon-shrink-right', ICON_SPREAD = 'layui-icon-spread-left'
    ,ICON_SHRINK = 'layui-icon-up' , ICON_SPREAD ='layui-icon-down'
  ...
  ,admin = {
    ...
    ,CheckMoblie:function(){
      ...
      if(isMobile){
          ...
          $('#LAY-system-side-menu').removeClass('layui-layout-right');
          $('#LAY-system-side-menu').addClass('layui-nav-tree');

          
      }
      else{

        $('#layui-side').removeClass('layui-side');
        $('#layui-side').removeClass('layui-side-menu');
        $('#layui-side-scroll').removeClass('layui-side-scroll');
      }
    }
    ,LoadMenu:function(){
      admin.req({
        //url: setter.ContextPath+'/assets/json/menu.json'
        url: setter.ContextPath+'/Menu.json?d=20190715'
        ,type: 'get'
        ,success: function(res){
          ...
          {
            var menu=res.data;
            var nav=$('[lay-filter=layadmin-layout-right]');
            console.log(menu);
//            console.log(nav);
            for (item in menu){
              var list='';
              if (menu[item].list.length>0){
                var listData=menu[item].list
                
                list=$('<dl class="layui-nav-child"></dl>');
                for (listItem in listData){
//                  list.append('<dd><a href="javascript:;" lay-href="index?page='+item+'_'+listItem+'#/'+item+'/'+listItem+'">'+listData[listItem].title+'</a></dd>')
                  list.append('<dd><a href="javascript:;" lay-href="index?page='+item+'_'+listItem+'#/'+item+'/'+listItem+'">'+listData[listItem].title+'</a></dd>')
                }
              }
              var li=menu[item].list.length>0?$('<li class="layui-nav-item"><a href="javascript:;">'+menu[item].title+'</a></li>'):$('<li class="layui-nav-item"><a href="javascript:;"lay-href="index?page='+item+'">'+menu[item].title+'</a></li>');
              li.append(list);
  ...
  $body.on('click', '*[lay-href]', function(){
    var othis = $(this)
    window.location= othis.attr('lay-href');
//    var othis = $(this)
//    
